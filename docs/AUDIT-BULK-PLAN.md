...
и сервера) — этого пункта в исходном плане не было. **Дополнено в тот же день:**
произвольные cron-расписания (per-site режим `cron` + воркспейс-дефолт
`defaultCron`, собственный матчер `src/lib/cron.ts`, маркер `lastFire`), 5+1 тестов
крон/precedence, 5 i18n-ключей ×7.
...
- Эффективное расписание сайта (`effectiveAuditSchedule` в `schedule.ts`): cron сайта →
  интервал сайта → cron воркспейса → интервал+час воркспейса; `off` выключает автоматику.
- Интервальный режим: на тике — по каждому сайту с интервалом: если нет живого
  (`running`/`queued`) аудита и последний завершённый старше интервала (воркспейс-дефолт
  или custom) → создать строку `trigger="scheduled"` и поставить в очередь. Час запуска
  (`scheduleHourUtc`) сглаживает все сайты к одному окну, как `hourUtc` у дайджестов.
- Cron-режим (5 полей, UTC, свой матчер `src/lib/cron.ts` без зависимостей, 5 тестов):
  `latestFireAtOrBefore` ищет последний матч в 60-минутном окне; маркер `lastFire` в
  `Site.auditSettings` — верхняя отметка. Первое знакомство только принимает маркер
  (без ретро-всплеска), даунтайм пропускается намеренно, а не копится; живой прогон
  придерживает маркер — окно догонит на следующем тике. Выражение валидируется на
  клиенте и сервере (`validateCron`); невалидное сохранённое трактуется как
  отсутствующее. Гранулярность срабатываний — тик 15 минут.
- «Next audit» не хранится: для интервальных сайтов колонка вычисляется как
  `lastFinished + interval` (для `mode="off"` — «—»), для cron-сайтов показывается само
  выражение — это честный ответ на «когда следующий».
