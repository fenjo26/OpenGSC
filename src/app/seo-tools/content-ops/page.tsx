...
type Operation = {
  ...
  createdAt: string; updatedAt: string; repository: Repo | null; events: Event[];
};
...
export default function ContentOperationsPage() {
  ...
  const [tab, setTab] = useState<"queue" | "repositories">("queue");
        ...
        <div className="content-ops-grid">
          ...
          {selected ? (
            <OperationDetail
              ...
              onCreatePr={createPr} onRefreshPr={refreshPr} t={t as any}
            />
          ) : <div className="panel" style={{ padding: 36, color: "var(--color-text-secondary)", textAlign: "center" }}>{t("contentOpsSelect" as any)}</div>}
        </div>
      )}
      <style jsx>{`
        .content-ops-grid { display:grid; grid-template-columns:minmax(270px, 0.8fr) minmax(420px, 1.6fr); gap:14px; align-items:start; }
        @media(max-width:900px){ .content-ops-grid{grid-template-columns:1fr;} }
      `}</style>
    </div>
  );
}
...
function OperationDetail(props: {
  ...
  onPreview: () => void; onCreatePr: () => void; onRefreshPr: () => void; t: (k: string) => string;
}) {
  const { operation, repositories, busy, preview, confirmPr, setConfirmPr, onSave, onDelete, onPreview, onCreatePr, onRefreshPr, t } = props;
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    {operation.status === "pr_merged" && <div className="panel">
      ...
      <p style={hintStyle}>{t("contentOpsMarkLiveHint")}</p>
      <button style={primaryButton} disabled={!!busy} onClick={() => onSave({ status: "live" }, "live")}><Check size={14} /> {t("contentOpsMarkLive")}</button>
    </div>}
    ...
    {["live", "measuring", "completed"].includes(operation.status) && <div className="panel">
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {operation.status === "live" && <button style={primaryButton} onClick={() => onSave({ status: "measuring" }, "measuring")}>{t("contentOpsStartMeasuring")}</button>}
        {operation.status === "measuring" && <button style={primaryButton} onClick={() => onSave({ status: "completed" }, "complete")}>{t("contentOpsComplete")}</button>}
      </div>
    </div>}
